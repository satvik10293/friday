...
def test_all_37_actions_registered():
    ...
    assert len(ALL_ACTION_SPECS) == 44          # + files & screen text/image locate+click
...
def test_tier_distribution():
    ...
    assert len(by_permission[Permission.SAFE]) == 33          # tiers 1 + 2
    assert len(by_permission[Permission.USER_APPROVAL]) == 6  # tier 3
