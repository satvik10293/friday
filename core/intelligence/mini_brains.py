...
class MathBrain(MiniBrain):
    ...
    _PCT = re.compile(r"([\d.]+)\s*%\s*of\s*([\d.]+)", re.IGNORECASE)
...
class ClockBrain(MiniBrain):
    ...
    _DATE = re.compile(r"\b(what('?s| is) (today'?s )?(the )?date|today'?s date)\b", re.IGNORECASE)
...
class UnitBrain(MiniBrain):
    ...
    _PAT = re.compile(
        r"([\d.]+)\s*(" + "|".join(sorted(_UNIT_ALIASES, key=len, reverse=True)) +
        r")\b\s*(?:to|in|into|as)\s*(" +
        "|".join(sorted(_UNIT_ALIASES, key=len, reverse=True)) + r")\b",
        re.IGNORECASE)
    ...
    def _parse(self, prompt: str):
        ...
        src = _UNIT_ALIASES[m.group(2).lower()]
        dst = _UNIT_ALIASES[m.group(3).lower()]
        if src == dst:
            return None
...
class RecallBrain(MiniBrain):
    ...
    def answer(self, prompt: str) -> Optional[str]:
        ...
        try:
            ...
            rows = search_keyword(topic, limit=3)
        except Exception:  # noqa: BLE001
            return None
        if not rows:
            return None
        snippets = "; ".join(r["content"][:120].replace("\n", " ") for r in rows)
...
class MiniBrainCortex:
    ...
    def __init__(self, brains: Optional[list[MiniBrain]] = None) -> None:
        self.brains: list[MiniBrain] = brains if brains is not None else [
            MathBrain(), ClockBrain(), UnitBrain(), SystemBrain(), RecallBrain(),
        ]
