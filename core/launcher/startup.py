    ...
    def _stage_voice(self):
        ...
        if ios is not None:
            ...
            try:                             # the notebook trick (M55): every cloud
                ...
                distiller = get_distiller()  # answer becomes a studied local note
                runtime = self.components.get("runtime")
                if distiller is not None and runtime is not None \
                        and self.start_runtime and hasattr(runtime, "schedule"):
                    dc = (self.config.get("distiller") or {})
                    runtime.schedule("distiller_cycle", distiller.run_cycle,
                                     float(dc.get("cycle_s", 600.0)))
            except Exception:  # noqa: BLE001 — the notebook is always optional
                pass
