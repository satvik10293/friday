...
| `core/launcher/` (M20) | `launcher` (`Launcher`), `startup`, `health`, `platform_adapter`, `logging_config` | Production launcher: OS detect → config → deps → ordered startup → health → recover. No cognitive logic. Entry: `friday_launch.py`. Docs: `docs/M20_DEPLOYMENT.md`. |
| `deploy/` (M20) | `install`, `build`, `release`, `version` | Installer + build/release scripts (cross-platform; secrets never embedded; verifiable packages). Artifacts in gitignored `dist/`. |
