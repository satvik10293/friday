...
| `core/brains/` (M17-rev) | `base` (`CognitiveBrain`/`SituationReport`/bus), `{vision,audio,spatial,memory,learning,emotion,automation,runtime,executive}/` | Cognitive Brains: each owns local reasoning/state/memory, emits Situation Reports. Memory Brain owns tiered memory + Knowledge Graph; Executive Brain (M18 foundation) consumes only Unified Situations. |
